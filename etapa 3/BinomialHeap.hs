...
attach tree1@(Node p1 k1 c1) tree2@(Node p2 k2 c2) =
  case p1 <= p2 of
    True -> Node p1 k1 ([tree2] ++ c1) -- Se ataseaza tree2 la tree1
    _ -> Node p2 k2 ([tree1] ++ c2) -- Se ataseaza tree1 la tree2
...
-}
isTreeEmpty :: BinomialTree p k -> Bool
isTreeEmpty EmptyTree = True
isTreeEmpty _ = False

numbChildren :: BinomialTree p k -> Int
numbChildren EmptyTree = 0
numbChildren tree@(Node p k c) = length c

insertTree :: Ord p => BinomialTree p k -> [BinomialTree p k] -> [BinomialTree p k]
insertTree tree trees = case trees of
  [] -> [tree]
  t : ts ->
    case isTreeEmpty t of
      True -> tree : ts -- Daca se intalneste o pozitie libera, se insereaza acolo.
      _ ->
        case ((numbChildren tree) < (numbChildren t)) of
          True -> tree : trees -- Are mai putini copii => poate fi inserat inainte
          _ -> case ((numbChildren tree) == (numbChildren t)) of
            True -> EmptyTree : (insertTree (attach tree t) ts)
            _ -> (attach tree t) : ts
...
-- BinomialTree se insereaza in vector si size-ul creste
insert prio key heap@(BinomialHeap size trees) =
  BinomialHeap
    (size + 1)
    (insertTree (Node prio key []) trees)
...
{-
    *** TODO ***
    ...
    Just (1,'a')
-}
prioCmp tree@(Node p k c) = p -- criteriu de ordonare
...
findMin heap@(BinomialHeap size trees) = case trees of
  ...
  where
    min@(Node p k c) =
      minimumBy -- minimul
        (compare `on` prioCmp)
        -- lista arborilor care nu sunt empty
        [x | x <- trees, isTreeEmpty x == False]
...
mergeTrees trees1 trees2 = f list 1
  where
    list = zipExtend EmptyTree EmptyTree trees1 trees2 -- perechi de BinomialTree
    f [] acc = [] -- daca lista e nula se returneaza null
    f (t : ts) acc =
      case isTreeEmpty (fst t) || isTreeEmpty (snd t) of -- Daca unul dintre arbori e gol, celalalt se ataseaza la restul listei
        True -> case acc > numbChildren (attach (fst t) (snd t)) of
          True -> attach (fst t) (snd t) : (f ts (2 * acc))
          _ -> EmptyTree : attach (fst t) (snd t) : (f ts (2 * acc)) -- Daca nu mai este loc pentru el, se adauga si un EmptyTree
        _ -> case acc > numbChildren (fst t) + numbChildren (snd t) + 2 of -- Mai au loc arbori pe pozitie
          True -> (insertTree (attach (fst t) (snd t)) (f ts (2 * acc))) -- Se insereaza arborii si se continua
          _ -> EmptyTree : (insertTree (attach (fst t) (snd t)) (f ts (2 * acc))) -- Se pune un EmptyTree si se continua
...
isolate placeHolder list =
  ( snd $
      mapAccumL
        -- elementul se inlocuieste cu perechea (1 => (1,[0,2,3]))
        (\acc x -> (acc + 1, (head (drop acc list), (take acc list) ++ [placeHolder] ++ (drop (acc + 1)) list)))
        0 -- cate elemente sunt inaintea elementului curent
        (take ((length list) - 1) list)
  )
    ++ [((last list), (init list))] -- ultima pereche
...
func tree@(Node prio key children) level =
  if null children
    then show prio ++ " (" ++ show key ++ ")" -- se afiseaza nodul frunza
    else
      show prio ++ " (" ++ show key ++ ")" ++ "\n"
        ++ ( intercalate "\n" $
               map (\c -> (replicate (2 * level) ' ') ++ (func c (level + 1))) children -- se afiseaza 2 * level spatii si se trece la nodurile copii
           )

instance (Show p, Show k) => Show (BinomialTree p k) where
  show EmptyTree = "*"
  show tree@(Node prio key children) = func tree 1
...
instance (Show p, Show k) => Show (BinomialHeap p k) where
  show heap@(BinomialHeap size trees) = if null trees then "" else intercalate "\n" (map show trees)
...
instance Functor (BinomialTree p) where
  ...
  fmap f tree@(Node prio key children) = (Node prio (f key) (map (fmap f) children))
...
instance Functor (BinomialHeap p) where
  ...
  fmap f heap@(BinomialHeap size trees) = (BinomialHeap size (map (fmap f) trees))
